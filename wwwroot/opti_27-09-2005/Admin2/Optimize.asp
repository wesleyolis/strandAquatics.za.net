...
if( Request.QueryString("DBName").count > 0 & Request.QueryString("Version").count > 0  & Request.QueryString("TMV").count > 0 )
{
	...
	filePath = "c:/Domains/strandaquatics.com/wwwroot/extration/" + database + ".mdb";
	oConn = Server.CreateObject("ADODB.Connection");
	if(Request.QueryString("TMV") != "TM4")
	{
	...
	else
	{
		...
		for(i=0;i<5 & ( conn == false);i++)
		{
			try
			{
				OpenString = "DRIVER={Microsoft Access Driver (*.mdb)};DBQ=" + filePath + ";";
				oConn.Open(OpenString);
...
if(fs.FileExists("c:/Domains/strandaquatics.com/wwwroot/extration/Swim4.mdb"))
{
fs.DeleteFile("c:/Domains/strandaquatics.com/wwwroot/extration/Swim4.mdb");
}	
Engine = Server.CreateObject("JRO.JetEngine");
Engine.CompactDatabase(OpenString, "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=c:/Domains/strandaquatics.com/wwwroot/extration/Swim4.mdb;Jet OLEDB:Database Password=");	
//fs.DeleteFile("c:/Domains/strandaquatics.com/wwwroot/extration/" + database + ".mdb");
Engine = null;
fs = null;
