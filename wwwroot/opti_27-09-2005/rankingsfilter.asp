...
/*
SQL+=" SELECT RESULT.RANK, RESULT.SCORE, Athlete.Last, Athlete.First, Int(DateDiff('d',[Athlete]![Birth],Date())/365.25) AS Ath_Age, [TEAM].[TCode] & '-' & [TEAM].[LSC] AS Team, MEET.MName";
SQL+=" FROM [SELECT Min(RESULT.RANK) AS MinOfRANK, RESULT.ATHLETE, RESULT.DISTANCE, RESULT.STROKE";
SQL+=" FROM RESULT";
SQL+=" WHERE (" + exclude_meet +")";
SQL+=" GROUP BY RESULT.ATHLETE, RESULT.DISTANCE, RESULT.STROKE, RESULT.COURSE, RESULT.I_R";
SQL+=" HAVING (((RESULT.COURSE)='"+Course+"') AND ((RESULT.I_R)='I'))";
SQL+=" ORDER BY RESULT.ATHLETE, RESULT.DISTANCE, RESULT.STROKE";
SQL+=" ]. AS st INNER JOIN (TEAM INNER JOIN ((Athlete INNER JOIN RESULT ON Athlete.Athlete = RESULT.ATHLETE) INNER JOIN MEET ON RESULT.MEET = MEET.Meet) ON TEAM.Team = Athlete.Team1) ON st.ATHLETE = RESULT.ATHLETE";
SQL+=" WHERE (((RESULT.RANK)=[st].[MinOfRANK] And (RESULT.RANK)<999) AND ((RESULT.SCORE)>0) " + SQLprov + " AND ((Int(DateDiff('d',[Athlete]![Birth],Date())/365.25))>=" + Lo + " And (Int(DateDiff('d',[Athlete]![Birth],Date())/365.25))<=" + Hi + ") AND ((Athlete.Athlete)=[st].[Athlete]) AND ((RESULT.DISTANCE)=[st].[DISTANCE] And (RESULT.DISTANCE)="+Distance+") AND ((RESULT.STROKE)=[st].[STROKE] And (RESULT.STROKE)="+Stroke+") AND ((Athlete.Sex)='"+Gender+"') AND ((RESULT.I_R)='I') AND ((RESULT.COURSE)='"+Course+"')  AND ((Athlete.Group)='A'))  AND (" + exclude_meet +")";
SQL+=" ORDER BY RESULT.SCORE;";*/


SQL+=" SELECT RESULT.SCORE, Athlete.Last, Athlete.First, Int(DateDiff('d',[Athlete]![Birth],Date())/365.25) AS Ath_Age, [TEAM].[TCode] & '-' & [TEAM].[LSC] AS Team, MEET.MName";
SQL+=" FROM [SELECT Min(RESULT.RANK) AS MinOfRANK, RESULT.ATHLETE, RESULT.DISTANCE, RESULT.STROKE";
SQL+=" FROM RESULT";
SQL+=" WHERE (" + exclude_meet +")";
SQL+=" GROUP BY RESULT.ATHLETE, RESULT.DISTANCE, RESULT.STROKE, RESULT.COURSE, RESULT.I_R";
...
SQL+=" ORDER BY RESULT.ATHLETE, RESULT.DISTANCE, RESULT.STROKE";
SQL+=" ]. AS st INNER JOIN (TEAM INNER JOIN ((Athlete INNER JOIN RESULT ON Athlete.Athlete = RESULT.ATHLETE) INNER JOIN MEET ON RESULT.MEET = MEET.Meet) ON TEAM.Team = Athlete.Team1) ON st.ATHLETE = RESULT.ATHLETE";
SQL+=" WHERE (((RESULT.RANK)=[st].[MinOfRANK] And (RESULT.RANK)<999) " + SQLprov + "AND ((Athlete.Athlete)=[st].[Athlete]) AND ((RESULT.DISTANCE)=[st].[DISTANCE] And (RESULT.DISTANCE)="+Distance+") AND ((RESULT.STROKE)=[st].[STROKE] And (RESULT.STROKE)="+Stroke+") AND ((Athlete.Sex)='"+Gender+"') AND ((RESULT.I_R)='I') AND ((RESULT.COURSE)='"+Course+"')  AND ((Athlete.Group)='A'))  AND (" + exclude_meet +")";

SQL+=" GROUP BY RESULT.SCORE, Athlete.Last, Athlete.First, Int(DateDiff('d',[Athlete]![Birth],Date())/365.25), [TEAM].[TCode] & '-' & [TEAM].[LSC], MEET.MName";
SQL+=" HAVING (((RESULT.SCORE)>0) AND ((Int(DateDiff('d',[Athlete]![Birth],Date())/365.25))>=" + Lo + " And (Int(DateDiff('d',[Athlete]![Birth],Date())/365.25))<=" + Hi + "))";
