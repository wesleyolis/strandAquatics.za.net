...
<%while(!rs.eof){%><tr><td><a href="meet_photos.asp?Meet=<%=rs.Fields("Meet")%>&MName=<%=rs.Fields("MName")%>"><%=rs.Fields("MName")%></a></td><td><%=rs.Fields("Start")%></td><td><%=rs.Fields("End")%></td><td align="center"><%=rs.Fields("Course")%></td>
...
function g(url)
{self.location.replace("meet_photos.asp?Meet=<%=Request.QueryString("Meet")%>&MtEvent=" + url+ "&MName=" + MName );}
function o(row)
...
<tr style="color: #000080; font-weight: bold" >
	...
	<a href="meet_photos.asp?Meet=<%=Request.QueryString("Meet")%>">Meet List (Back)</a></td> 
...
if (!rs.eof & Request.QueryString("MtEvent") != "ALL") 
{
	var oConn2;	
	var rs2;		
	var filePath2;	
	filePath2 = Server.MapPath("../web.mdb");
	oConn2 = Server.CreateObject("ADODB.Connection");
	oConn2.Open("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" +filePath2);

	rs2 = oConn2.Execute("SELECT AEvent.* FROM AEvent WHERE (((AEvent.MEvent)=" + Request.QueryString("MtEvent") + "));");
}
...
<table border="0" cellpadding="0" cellspacing="0" width="799" id="table1">
	...
	<tr>
	<td align="center">
	<a href="meet_photos.asp?Meet=<%=Request.QueryString("Meet")%>&MName=<%=Request.QueryString("MName")%>">Events List (Back)</a></td></tr>
	<%if (!rs2.eof & Request.QueryString("MtEvent") != "ALL") {%>
	<tr><td>&nbsp;</td></tr>
	<%
	if(rs2("Photo") == true){
	%>
	<tr><td align="center">
		<a title="Click to large photo" href="photos/<%=rs2("MEvent")%>.jpg">
		<img border="2" src="../meets/photos/<%=rs2("MEvent")%>.jpg" width="491" height="264"></a></td></tr>
		<%}%>
	<tr><td align="center">
		<table border="0" width="499px" id="table4" cellspacing="0" cellpadding="3">
			<tr>
				<td align="center"><b><%=rs2("Subtext")%></b></td>
			</tr>
		</table>
		</td></tr>
		<%
