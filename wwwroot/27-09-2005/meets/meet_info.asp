...
var strokes = new Array("","Freestyle","Back","Breast","Butterfly","Medley");
	...
	{
	rs = oConn.Execute("SELECT MEET.MName, MEET.Course, MEET.[Start], MEET.AgeUp, MEET.[End], IIf(IsNull([Since]),'Not Applicable',[Since]) AS Sinces, IIf([RestrictBest]=True,'Yes','No') AS RestrictBests, IIf(([MinAge]=0) Or (IsNull([MinAge])),'None',[MinAge]) AS MinAges, MEET.Location, MEET.Remarks, 'R ' & IIf(IsNull([IndCharge]),'0',[IndCharge]) AS IndCharges, 'R ' & IIf(IsNull([SurCharge]),'0',[SurCharge]) AS SurCharges, 'R ' & IIf(IsNull([RelCharge]),'0',[RelCharge]) AS RelCharges, 'R ' & IIf(IsNull([TeamFee]),'0',[TeamFee]) AS TeamFees, 'R ' & IIf(IsNull([FacilityFee]),'0',[FacilityFee]) AS FacilityFees, MEET.Instructions FROM MEET  WHERE (MEET.Meet =" + Request.QueryString("Meet") + ");");
%>
...
<font face="Arial Rounded MT Bold" size="6" color="#000080">&nbsp;<%=rs.Fields("MName")%> - <%=rs.Fields("Course")%></font></div></td></tr><tr><td align='center' valign='top'>
<div align="left">
