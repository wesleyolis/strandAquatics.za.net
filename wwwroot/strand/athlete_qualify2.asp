...
<tr height="10"><td width="75"></td><td width="45">q/stand</td><td width="65">
	q/time</td><td width="65"></td><td width="104"></td><td></td></tr>
<%
while(!rs.eof)
{
...
//goal = "<tr height ='20px' valign='top' align='left' style='padding-top: 6px'><td colspan='10'>My Goal Time is,&nbsp;&nbsp;" + rs.Fields("NNext Level").Value + "&nbsp;-&nbsp;" + CTime(rs.Fields("Next Level").Value) + "&nbsp; must improve by&nbsp;" + CTime(rs.Fields("Improve").Value) + "s</td></tr>";
 ...
 %><tr  height="30" valign="bottom">
	<td colspan="4" class='dashborder' height="33"><%=Distance%>&nbsp;<%=strokes[Stroke] %></td>
	<td class='dashborder' colspan="2" height="33" ><%=goal%></td></tr><%}
	
%>
<tr><td><%=CTime(rs.Fields("SCORE").Value)%></td><td><%=rs.Fields("STD").Value%></td><td><%if(rs.Fields("Current Level").Value != null ){Response.write(CTime(rs.Fields("Current Level").Value))}%></td><td><%if(rs.Fields("F_P").Value=="F"){%>Final<%}else{%>Prelim<%}%></td><td><%=Server.HTMLEncode(rs.Fields("End"))%></td><td><%=Server.HTMLEncode(rs.Fields("MName"))%></td></tr><%
rs.MoveNext();}
rs.close();
}%>
<%
