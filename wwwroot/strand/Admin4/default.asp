		...
		- Update Database</b></font><form name="updb" method="POST" action="default.asp">
			...
			<input type="text" name="Version" size="4"></td>
				...
				<tr>
					...
					function convert()
					{
						var pos = 0,prev = 0, msg="";
						while(pos !=-1)
						{
							
							msg += document.updb.Message.value.substring(prev,pos) + "<br>";
							pos = document.updb.Message.value.indexof("\n", pos);
						}
						document.updb.Message.value = msg;
					}
					//-->
					</script>
					<textarea onchange="convert();" rows="10" name="Message" cols="56"></textarea></td>
				</tr>
				<tr>
...
//Server.Execute("update_wp.asp");//?DBName=" + Request.Form("DBName") + "&Version=" + Request.Form("Version") + "&TMV=" + Request.Form("TMV"));
Server.Execute("extra_database.asp");//?DBName=" + Request.Form("DBName") + "&Version=" + Request.Form("Version") + "&TMV=" + Request.Form("TMV"));
//Server.Execute("Optimize.asp");//?DBName=" + Request.Form("DBName") + "&Version=" + Request.Form("Version") + "&TMV=" + Request.Form("TMV"));
Server.Execute("Email_notify.asp");//?DBName=" + Request.Form("DBName") + "&Version=" + Request.Form("Version") + "&TMV=" + Request.Form("TMV"));
